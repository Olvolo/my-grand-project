<?php $__env->startSection('content'); ?>
    <h1 class="text-4xl font-extrabold text-gray-900 mb-8 text-center">Все Книги</h1>

    <?php if($books->isNotEmpty()): ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php $__currentLoopData = $books; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $book): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="bg-white p-6 rounded-lg shadow-md flex flex-col">
                    <?php if($book->cover_image): ?>
                        <div class="mb-4 flex justify-center">
                            <img src="<?php echo e(asset('storage/' . $book->cover_image)); ?>" alt="<?php echo e($book->title); ?>"
                                 class="w-32 h-48 object-cover rounded-md shadow-lg">
                        </div>
                    <?php endif; ?>
                    <h2 class="text-xl font-semibold text-gray-800 mb-2">
                        <a href="<?php echo e(route('books.show', $book)); ?>" class="hover:text-indigo-600">
                            <?php echo e($book->title); ?>

                        </a>
                        <?php if($book->is_hidden): ?>
                            <span class="ml-2 px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs font-semibold">Скрыто</span>
                        <?php endif; ?>
                    </h2>
                    <div class="text-gray-600 text-sm mb-3">
                        <?php if($book->publication_year): ?>
                            <span>Год: <?php echo e($book->publication_year); ?></span>
                        <?php endif; ?>
                        <?php if($book->authors->isNotEmpty()): ?>
                            <span class="mx-1">|</span> Авторы:
                            <?php $__currentLoopData = $book->authors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $author): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <a href="<?php echo e(route('authors.show', $author)); ?>" class="hover:underline">
                                    <?php echo e($author->name); ?>

                                </a><?php if(!$loop->last): ?>, <?php endif; ?>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php endif; ?>
                        <?php if($book->chapters_count > 0): ?>
                            <span class="mx-1">|</span> Глав: <?php echo e($book->chapters_count); ?>

                        <?php endif; ?>
                    </div>
                    <?php if($book->description): ?>
                        <p class="text-gray-700 text-sm line-clamp-3 mb-4">
                            <?php echo Str::limit(strip_tags($book->description), 150); ?>

                        </p>
                    <?php endif; ?>
                    <div class="mt-auto text-right">
                        <a href="<?php echo e(route('books.show', $book)); ?>"
                           class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 transition duration-300">
                            Перейти к книге
                            <svg class="ml-2 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
                        </a>
                    </div>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    <?php else: ?>
        <p class="text-center text-gray-600 text-lg">Пока нет добавленных книг.</p>
    <?php endif; ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH N:\www\my-grand-project\resources\views/books/index.blade.php ENDPATH**/ ?>